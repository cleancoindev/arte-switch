var Switch = React.createClass({
    ...
    render() {
        ...
        if (this.props[this.props.i].startBlock >= this.props[this.props.i].currentBlock) {
            return (<section className="switchBox">
                <h3>Switch</h3>
                <section className="switchTools">
                    <h4>The Switch will be available at block <a target="_blank" href={window.getNetworkElement("etherscanURL") + "block/countdown/" + this.props[this.props.i].startBlock}>#{this.props[this.props.i].startBlock}</a></h4>
                </section>
            </section>);
        }
        return (<section>
            <section className="switchBox">
                ...
                <section className="switchActions">
                    {!this.props[this.props.i].currentSlot && <Loader />}
                    {this.props[this.props.i].currentSlot && window.walletAddress && <a href="javascript:;" className={"switchAction" + (!this.props[this.props.i].approved ? " active" : "")} onClick={this.approve}>Approve</a>}
                    {this.props[this.props.i].currentSlot && window.walletAddress && <a href="javascript:;" className={"switchAction" + (this.props[this.props.i].approved ? " active" : "")} onClick={this.switch}>Switch</a>}
                    {this.props[this.props.i].currentSlot && !window.walletAddress && <a href="javascript:;" onClick={() => window.ethereum.enable().then(() => window.getAddress()).then(() => _this.emit('ethereum/ping'))} className="switchAction active">Connect</a>}
                </section>
                {this.props[this.props.i].currentSlot && <p>Current Switch bonus is <b>X{window.numberToString(parseInt(this.props[this.props.i].currentSlot[1]) / parseInt(this.props[this.props.i].currentSlot[2]))}</b> until the block n. <a href={window.getNetworkElement("etherscanURL") + "block/" + this.props[this.props.i].currentSlot[0]} target="_blank">{this.props[this.props.i].currentSlot[0]}</a></p>}
                {this.props[this.props.i].currentSlot && <p>Disclamer: Switching $BUIDL V1 to $buidl V2 is an irreversible action, do it at your own risk</p>}
            </section>
        </section>);
    }
});